...
describe("Cart test", () => {
  ...
  describe("Checkout", () => {
    ...
    it("should throw 404 error if cart is not present", async () => {
      ...
      try {
        await cartService.checkout(userOne);
      } catch (error) {
        /* - ApiError is thrown
        ...
        expect(error.message).toEqual("User does not have a cart");
      }
    });
    ...
    it("should throw 400 error if address is not set - when User.hasSetNonDefaultAddress() returns false", async () => {
      ...
      try {
        await cartService.checkout(userTwo);
      } catch (error) {
        // - ApiError is thrown
        // - the "statusCode" field of response is "400 BAD REQUEST"
    ...
    it("should throw 400 error if wallet balance is insufficient", async () => {
      ...
      try {
        await cartService.checkout(userOneWithZeroBalance);
      } catch (error) {
        // - ApiError is thrown
        // - the "statusCode" field of response is "400 BAD REQUEST"
        expect(error.statusCode).toBe(httpStatus.BAD_REQUEST);
    ...
    it("should update user balance and empty the cart on success", async () => {
      ...
      const amountToDeduct = cartWithProductsUserOne.cartItems.reduce(
        (acc, item) => acc + item.product.cost * item.quantity,
        0
      );
      expect(userOneFinal.walletMoney).toEqual(
        userOne.walletMoney - amountToDeduct
      );
    });
  });
});
