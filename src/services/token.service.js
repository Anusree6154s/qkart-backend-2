...
const generateToken = (userId, expires, type, secret = config.jwt.secret) => {
  return jwt.sign({ exp: expires, sub: userId, type }, secret);
};
...
const generateAuthTokens = async (user) => {
  let expiry = 3600 * 1000; //60min*60sec = 1 hour in seconds
  let date = new Date(Date.now() + expiry);

  let token = generateToken(user._id, Math.floor(date / 1000), "access"); // divide by 1000 to convert from millisec to sec

  return { access: { token, expires: date } };
};
